...
${nila + bawal + ikan + lele}`
        ...
        conn.reply( m.chat, mcng, m)
         ...
         conn.sendBut( m.chat, `You Get 🎁chest weapons epic ${psepick} item`, `games wa bot`, `again`, `.mancing`, m)
         } 
        if  (psenjata > 0 ) {
        	global.db.data.users[m.sender].psenjata+= psenjata * 1
        conn.sendBut( m.chat, `You Get 🎁chest weapons ${psenjata} item`, `games wa bot`, `Again`, `.mancing`, m)
        	}
        }, 38000)
