...
let date = global.wm
    ...
    let pp = 'https://www6.flamingtext.com/net-fu/proxy_form.cgi?&imageoutput=true&script=sketch-name&doScale=true&scaleWidth=300&scaleHeight=300&fontsize=100&fillTextType=1&fillTextPattern=Warning!&text=Inventory'
    const ftrol = {
    key : {
...
🦑 *Cumi:* ${cumi}
🦐 *Udang:* ${udang}
💬 *Total Ikan:* ${orca + udang + hiu + lobster + lumba + cumi + gurita + kepiting + paus  + bawal + ikan + lele }
*──────────❏ Inventory ❏──────────*
💎 *Diamond:* ${diamond}
🥤 *Potion:* ${potion}
🗑️ *Sampah:* ${sampah}
...
`
    conn.send2ButtonLoc(m.chat, await(await fetch(fla + 'INVENTORY')).buffer(), str, wm, `${healt < 40 ? 'Heal' : 'Adventure'}`, `${healt < 40 ? '.heal' : '.adventure'}`, 'Shop', '.shop', ftrol)
}
